...
import Control.Monad.Trans.Reader (Reader, ReaderT(runReaderT), ask, local, runReader)
...
class Monad m => GraphMonad m where
  addNode :: Node -> m NodeIndex
  getNode :: NodeIndex -> m Node
  updateNode :: NodeIndex -> NodeIndex -> m ()

instance GraphMonad (State Graph) where
...
instance GraphMonad m => GraphMonad (ReaderT r m) where
  addNode :: Node -> ReaderT r m NodeIndex
  addNode = lift . addNode

  getNode :: NodeIndex -> ReaderT r m Node
  getNode = lift . getNode

  updateNode :: NodeIndex -> NodeIndex -> ReaderT r m ()
  updateNode = (lift .) . updateNode


-- | Converts digits generated by alpha renaming to subscripts for better display.
digitToSubscript :: Char -> Char
digitToSubscript c
  | isDigit c = toEnum $ fromEnum '₀' - fromEnum '0' + fromEnum c
  | otherwise = c
...
copy :: GraphMonad m => NodeIndex -> m NodeIndex
copy index = do
  node <- getNode index
  copyNode node

-- | Copies a subgraph and appends it to the end of the graph.
copyNode :: GraphMonad m => Node -> m NodeIndex
...
instantiate param body arg = do
  ...
  if paramInBody
    then getNode body >>= \case
      VarNode v
        | v == param -> if strat == CallByNeed then return arg else copy arg
